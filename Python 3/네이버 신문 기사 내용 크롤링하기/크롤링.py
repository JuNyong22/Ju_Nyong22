...
# 긁어 올 URL
URL = 'https://news.naver.com/main/read.nhn?mode=LSD&mid=shm&sid1=100&oid=001&aid=0010985281'
...
def get_company(URL):
    ...
    if mystr.find(search1) == -1:
        indexno = mystr.find(search2)
    else:
        indexno = mystr.find(search1)
    ...
    all_newscompany = ["경향신문", "국민일보", "노컷뉴스", "뉴데일리", "뉴스타파", "뉴시스", "데일리안", "동아일보",
                            ...
                            "중부일보", "충북일보", "충청일보", "충청투데이", "한라일보", "연합뉴스"]
...
def main():
    ...
    #파일이 없으면 생성
    RESULT_PATH = 'D:/크롤링/' + str(result_company) + '/'
    if not os.path.isdir(RESULT_PATH):
        os.mkdir(RESULT_PATH)

    # 출력 파일 명
    OUTPUT_FILE_NAME = RESULT_PATH + result_date + '분 ' + str(result_company) + " " + result_title + '.txt'
